...
export default function CalculatorPage() {
  ...
  const handleAddFinanceOption = async () => {
    try {
      ...
      const newFinanceOption = {
       ...
       if (user) {
        // Save the new finance option to Firestore under the user's document
        const userRef = doc(db, "users", user.uid);
        const financeOptionsCollection = collection(userRef, "financeOptions");

        await addDoc(financeOptionsCollection, newFinanceOption);
      }
  ...
  const handleDeleteFinanceOption = async (index: number) => {
    ...
    try {
      ...
      // Get all documents in the financeOptions collection
      const querySnapshot = await getDocs(financeOptionsCollection);
      querySnapshot.forEach(async (document) => {
        const data = document.data();
        // Delete where data is equal to the option
        if (
          data.financeSourceName === optionToDelete.financeSourceName &&
          data.loanAmount === optionToDelete.loanAmount &&
          data.annualInterestRate === optionToDelete.annualInterestRate &&
          data.insuranceRatePercentage === optionToDelete.insuranceRatePercentage &&
          data.insuranceAmount === optionToDelete.insuranceAmount &&
          data.securityDeposit === optionToDelete.securityDeposit &&
          data.monthlyRepaymentAmount === optionToDelete.monthlyRepaymentAmount &&
          data.monthlyRepaymentIsPercentage === optionToDelete.monthlyRepaymentIsPercentage &&
          data.loanTermMonths === optionToDelete.loanTermMonths &&
          data.loanAmountPaidAtTermEnd === optionToDelete.loanAmountPaidAtTermEnd &&
          data.securityDepositRepayable === optionToDelete.securityDepositRepayable &&
          data.canRenew === optionToDelete.canRenew &&
          data.loanRenewalPercentage === optionToDelete.loanRenewalPercentage &&
          data.loanRenewalFixedCost === optionToDelete.loanRenewalFixedCost
        ) {
          const docRef = doc(db, "users", user.uid, "financeOptions", document.id);
          await deleteDoc(docRef);
        }
      });
    } catch (error: any) {
      console.error('Error deleting finance option:', error.message);
      alert('Failed to delete finance option.');
    }
  };
  ...
  return (
    <div className="container mx-auto p-4">
        <Button onClick={() => router.push('/report')}>Go to Comparative Report</Button>
